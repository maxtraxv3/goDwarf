...
func Load(path string) (*CLImages, error) {
	...
	parsed, perr := parseCLImages(data)
	if perr != nil {
		closer.Close()
		return nil, perr
	}
	parsed.closer = closer
	runtime.SetFinalizer(parsed, func(c *CLImages) { c.Close() })
	return parsed, nil
}

// Close releases the backing file of a mapped archive. Safe to call multiple
// times; a no-op for archives loaded from memory.
func (c *CLImages) Close() {
	c.closeOnce.Do(func() {
		if c.closer != nil {
			c.closer.Close()
			c.closer = nil
...
// SetMaxCache limits the number of decoded artwork textures retained in the
// cache. When the cache grows past the limit the oldest half is evicted and
// re-decoded on demand. A value of 0 disables the limit.
func (c *CLImages) SetMaxCache(n int) {
	c.mu.Lock()
	c.maxCache = n
	if n > 0 && len(c.cacheOrder) > n {
		evict := len(c.cacheOrder) - n
		for i := range evict {
			k := c.cacheOrder[i]
			if img := c.cache[k]; img != nil {
				img.Deallocate()
			}
			delete(c.cache, k)
		}
		c.cacheOrder = append([]string(nil), c.cacheOrder[evict:]...)
	}
	c.mu.Unlock()
}
...
func (c *CLImages) Get(id uint32, custom []byte, forceTransparent bool) *ebiten.Image {
	...
	if c.maxCache > 0 && len(c.cacheOrder) > c.maxCache {
		evict := len(c.cacheOrder) / 2
		for i := range evict {
			k := c.cacheOrder[i]
			if old := c.cache[k]; old != nil {
				old.Deallocate()
			}
			delete(c.cache, k)
		}
