...
func TestScriptAPIFull(t *testing.T) {
    ...
    foundCtrlU := false
    for _, hk := range list {
        if hk.Combo == "Ctrl-Alt-F" { foundKey = true }
        if hk.Combo == "Ctrl-U" { foundCtrlU = true }
    }
